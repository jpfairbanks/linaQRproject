...
%    &= C_1
\section{Testing}
Our input space has three dimensions: the number of rows of $A$, $m$; the number of columns of $A$, $n$; and, the number of nonzero columns in $\Delta A$, $k$.  We examined values of $m$ of 100, 500, and 1000.  Based on the value of $m$, we chose the values of $n$ and $k$ using the following method:
\begin{align*}
    \{n &= 2^i \mid 1\le i \le \log{\left(m\right)}\}\\
    \{k &= 2^i \mid 1 \le i \le \log{\left(n/2\right)}\}
\end{align*}
...
\begin{enumerate}
\item Generate random $m\times n$ matrix $A$
\item Generate $k$ random $m\times 1$ vectors 
\item Place the $k$ vectors into random columns of $\Delta A$
\item Execute each QR Algorithm on $A$+$\Delta A$	
\item Repeat Steps (1)-(4) 30 times
\item Determine median time to perform Step (4)
...
in Figures~\ref{fig:100arc}-~\ref{fig:1000arc} we see that there is nearly linear growth in $r(m_0, n, k_0)$ as $n$ increases, when $m$ is large. This is what we would expect from the analytic bounds on flop counts.
